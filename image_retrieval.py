...
class ColpaliRetrieval(BaseRetrieval):
    ...
    def __init__(self, config):
        self.config = config
        if ColpaliRetrieval._shared_model is None:
            model_name = "vidore/colpali"
            model = ColPali.from_pretrained(
                "vidore/colpaligemma-3b-mix-448-base",
                torch_dtype=torch.bfloat16, # temp dtype the original is float32
                device_map="auto",
            ).eval()
            model.load_adapter(model_name)
            ColpaliRetrieval._shared_model = model
            ColpaliRetrieval._shared_processor = AutoProcessor.from_pretrained(model_name)

        self.model = ColpaliRetrieval._shared_model
        self.processor = ColpaliRetrieval._shared_processor
